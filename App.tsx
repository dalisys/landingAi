  ...
  useEffect(() => {
    const checkApiKey = async () => {
      if ((window as any).aistudio) {
        ...
        // Fallback for local development or environments without aistudio object
        setHasApiKey(true);
      }
    };
    checkApiKey();
          ...
          {(viewStatus === AppStatus.GENERATING_IMAGES || (viewStatus === AppStatus.GENERATING_CODE && project.sections.some(s => !s.generatedCode))) && (
             <div className="animate-fade-in">
               <div className="text-center mb-8">
                  <div className="inline-block mb-4 px-4 py-2 border border-cyan-500/30 bg-black/50 backdrop-blur-sm" style={{ fontFamily: 'var(--font-mono)', fontSize: '12px' }}>
                    <span className="neon-text-cyan">&gt;_</span> <span className="text-cyan-300/60">PROCESS:</span> <span className="neon-text-magenta">VISUAL_SYNTHESIS</span>
                  </div>
                  <h2 className="text-2xl md:text-4xl font-bold neon-text-cyan mb-2" style={{ fontFamily: 'var(--font-display)' }}>{getStatusMessage(AppStatus.GENERATING_IMAGES, displaySensitiveData).title}</h2>
                  <p className="text-cyan-100/70" style={{ fontFamily: 'var(--font-body)' }}>{getStatusMessage(AppStatus.GENERATING_IMAGES, displaySensitiveData).subtitle}</p>
               </div>
               <Gallery
